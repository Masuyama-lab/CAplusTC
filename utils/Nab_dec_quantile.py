...
Adapted from Cognet-74/TorqueClusteringPy:
...
def Nab_dec_quantile(
    ...
    # Convert inputs to float64 for consistency with MATLAB
        # Convert inputs to 1D float64 arrays for consistency with MATLAB
    ...
    # ==========================
    # Mode 1: torque-rank-based selection
    # ==========================
    if mode == "torque_band":
        # Count only valid (non-NaN) torque values
        valid_mask = ~np.isnan(sort_p_1)
        n_valid = int(np.sum(valid_mask))

        if n_valid == 0:
            # If all torque values are NaN, no edges can be cut
            NAB = np.array([], dtype=np.int64)
            resolution = np.array([], dtype=np.int64)
            diagnostics['torque_band_info'] = {
                'n_valid': 0,
                'cut_k': 0
            }
            return NAB, resolution, diagnostics

        # Determine the number of edges to cut from torque_band (e.g., 0.25, 0.5, 0.75)
        frac = float(torque_band)
        frac = max(0.0, min(1.0, frac))  # Clip to [0, 1] for safety

        k = int(np.floor(n_valid * frac))
        if k < 1:
            k = 1  # Cut at least one edge

        # Treat the top k edges in descending torque order as cut targets
        # resolution: descending-order indices 0, 1, ..., k-1
        resolution = np.arange(k, dtype=np.int64)

        # For compatibility, return NAB as a one-element array containing the last edge position
        NAB = np.array([k - 1], dtype=np.int64)
